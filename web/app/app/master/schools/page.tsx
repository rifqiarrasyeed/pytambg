...
type School = { id: string; code: string; name: string; address: string | null };
...
export default function MasterSchoolsPage() {
  ...
  const load = async () => {
    const res = await apiFetch<{ data: School[] }>("/api/app/schools?page=1&page_size=50");
    setRows(res.data);
  };
  ...
  const create = async () => {
    await apiFetch("/api/app/schools", { method: "POST", body: JSON.stringify({ code, name }) });
    setCode("");
    setName("");
    await load();
  };
  ...
  return (
    <div className="space-y-4 pb-20 md:pb-0">
      <Card>
        <CardHeader><CardTitle>Master Sekolah</CardTitle></CardHeader>
        <CardContent className="grid gap-2 md:grid-cols-[1fr_2fr_auto]">
          ...
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nama sekolah" />
